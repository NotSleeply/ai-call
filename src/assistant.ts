...
└─────────────────────────────────────────────────────────────┘
   ...
   * 递归搜索目录
   */
  private searchInDirectory(dir: string, keyword: string): Array<{file: string, line: number, content: string}> {
...
💡 项目类型: ${this.detectProjectType(stats.fileTypes)}
  ...
  async askQuestion(question?: string): Promise<void> {
    ...
    const answers: Record<string, string> = {
      'typescript': 'TypeScript是JavaScript的超集，添加了静态类型检查和面向对象编程特性。它编译成纯JavaScript，可以在任何浏览器和Node.js环境中运行。',
      ...
      'default': '这是一个很好的问题！在实际的大虾助手中，我会使用先进的AI模型来回答你的问题。这个Demo只是展示了基本的功能框架。'
    };
  ...
  async smartChat(input: string): Promise<void> {
    // 检测用户意图
    if (input.includes('什么') || input.includes('如何') || input.includes('怎么')) {
