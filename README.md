...
管道内容会作为上下文与提问合并；回答只进 stdout，可直接重定向或继续管道。模型产生最终回答时会按流式增量输出。

### PowerShell：报错、多行文本与引号

原生命令的报错通常写到 stderr，而 PowerShell 的 `|` 默认只传递 stdout。要把 Git 等命令的完整报错交给 `aic`，先用 `2>&1` 合并两个输出流：

```powershell
git switch -b feat/2026-09-02-auto-list-edit 2>&1 |
  aic '解释这段 Git 报错，并给出正确命令'
```

这里的 `git switch -b` 会故意报错，因为 `git switch` 创建分支应使用 `-c`；它适合作为分析报错的示例。不要用 `>` 连接 `aic`：`>` 会把 stdout 重定向到名为 `aic` 的文件，而不会启动 `aic`。

传入多行内容时，优先使用单引号 here-string。它保留换行，不展开变量或反引号，内容中可以直接包含单双引号：
...
结束标记 `'@` 必须从新行的行首开始；其后可以继续接管道。短文本可按下面的规则引用：
...
- 交互终端会显示正在分析、查找文件、读取文件、搜索内容和整理回答等阶段状态
- API 连接阶段 30 秒没有响应，或流式响应连续 30 秒没有新内容，会自动超时
- 按 `Ctrl+C` 会中断当前请求，不重试，也不会保存半截回答；已经输出到终端的内容无法撤回
