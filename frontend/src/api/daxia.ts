...
export const daxiaAPI = {
  ...
  async getConversations(limit: number = 20): Promise<Conversation[]> {
    const response = await axios.get(`${API_BASE_URL}/conversations`, { params: { limit } })
  ...
  // 执行命令
  async executeCommand(command: string, conversationId?: number): Promise<CommandResponse> {
    try {
